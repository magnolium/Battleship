...
function SetCountry(cname)
{
    //alert(cname);
    document.cookie = cname + "=checked";
    SetCountryX(cname.replace("chk_","xf_"));
}
...
function startMap () 
{
    ...
    $.ajax({
        ...
        success: function (data) {
			...
			//console.log("SX: " + mapWidth+" : " + mapHeight);
            ...
            $xml.find('stateData').each(function (i) 
            {
                ...
                bfirst = false;
                var $node = $(this);
                //console.log("SN: "+$node.attr('stateName'));
                stateText.push($node.text());
                stateNames.push($node.attr('stateName'));
                var cntry = "chk_" + $node.attr('stateName').replace(/ /g, "");

                //stateURLs.push($node.attr('url'));
                stateURLs.push('javascript:SetCountry("'+cntry+'")');
                ...
                if (getCookie(cntry) == "checked")
                {
                    stateColors.push('#47A447');
                    //console.log($node.attr('stateName')+": Checked");
                }
                else
                    stateColors.push('#94B09E');
    ...
    function createMap() 
	{
        ...
        for (var state in paths)
        {
            ...
            arr[obj.id] = state;
            //console.log(state+"["+obj.id+"]");
            if (stateModes[obj.id] == 'OFF')
            {
                //console.log("FILL: OFF :"+obj.id);
                obj.attr({
                    fill: offColor,
                    cursor: 'default'
                });
            ...
            else
            {
                ...
                obj.mouseover(function (e) {
                    ...
                    if (!isPanning) {
                        ...
                        //console.log(stateNames[this.id]);
